...
serve(async (req) => {
  ...
  try {
    // Get the plan from the request data
    const { plan } = await req.json();
    
    if (!plan || !['subscriber', 'admin'].includes(plan)) {
    ...
    const priceId = plan === 'admin' 
      ? Deno.env.get("STRIPE_ADMIN_PRICE_ID") 
      : Deno.env.get("STRIPE_SUBSCRIBER_PRICE_ID");
